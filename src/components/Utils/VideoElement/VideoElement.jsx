...
export function VideoElement({ src, type }) {
    ...
    return (
        <div className="h-full w-full">
            <video ref={elementRef} width={width} height={height} autoPlay muted controls playsInline loop className="rounded-sm border border-black object-fill w-full h-full max-w-[768px]">
                <source src={src}
                    type={`video/${type}`} />
            </video>
        </div>
