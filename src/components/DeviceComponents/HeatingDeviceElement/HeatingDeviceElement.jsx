...
export function HeatingDeviceElement({ device }) {
  ...
  function getSliderTemplate(attribute) {
    return (
      <SliderElement attribute={attribute.name}>
        <SliderScala
          scala={attribute.scala}
          min={attribute.min}
          max={attribute.max}>
          <Slider
            colorClass={device.cube}
            state={attribute.value}
            scala={attribute.scala}
            disabled={attribute.disabled}
            callback={(value) => updateAttributeValue(attribute, value)}
          />
        </SliderScala>
      </SliderElement>
    );
  ...
  function getToggleTemplate(attribute) {
    return (
      <ToggleElement attribute={attribute.name}>
