...
export function Humidity({state, outline, base, fill}) {
    ...
    return <div>
        <svg width="44" height="46" viewBox="0 0 44 46" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <g>
                <path className={`fill-current ${outline}`} transform-origin="top" transform={`translate(${8 - 8*state} ${2 - 14.8 * state}) scale(${1 + 0.54 * state} ${1 + 0.6 * state})`} d="M27 28.3333C27 30.9093 24.7614 33 22 33C19.2386 33 17 30.9093 17 28.3333C17 24.4673 22 19 22 19C22 19 27 24.4673 27 28.3333Z" fill="#0B0B1C"/>
                <path className={`fill-current ${fill}`} transform-origin="top" transform={`translate(${8 - 8*state} ${2 - 25.8 * state}) scale(${1 + 1 * state})`} d="M25 28C25 29.656 23.6569 31 22 31C20.3431 31 19 29.656 19 28C19 25.5147 22 22 22 22C22 22 25 25.5147 25 28Z" fill="#707FDC"/>
            </g>
        </svg>
    </div>
};
